...
$check_all_signs2 = (new \App\Models\DirectorSign())->where('session', active_session())->where('class',$class->id)->where('section',$section->id)->where('is_signed', 1)->get()->getLastRow();
...
<div class="table-responsive" style="overflow-x:auto;">
    <table class="table">
        ...
        <tr>
            ...
            <th style="width: 10%">
                Homeroom Teacher Sign
                <input type="checkbox" name="homeroom_sign" value="all" id="checkAll" <?php if(empty($check_all_signs) && !empty($check_all_signs2)){?> checked <?php }?>>
            </th>
        </tr>
        </thead>
        <tbody>
           ...
           // $d_sign = (new \App\Models\DirectorSign())->where('session', active_session())->where('class',$class->id)->where('section',$section->id)->where('student', $student->id)->get()->getLastRow();
            ...
            foreach ($semesters as $item) {
                foreach ($subjects as $subject) {
                    ...
                    if ($result && !empty($result) && $subject->optional == 0) {
                        $sub_count++;
                        if (isset($total_marks[$student->id.'.'.$item->id])) {
                            $total_marks[$student->id.'.'.$item->id] += is_numeric($result) ? $result : 0;
                        } else {
                            $total_marks[$student->id.'.'.$item->id] = is_numeric($result) ? $result : 0;
                        }
                    }
            ...
            <tr>
                ...
                <td style="text-align: center">
                    <a class="btn btn-sm btn-info" href="<?php echo site_url(route_to('teacher.academic.yearly_certificate.report-card', $student->id)); ?>">View/Edit</a>
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <form class="ajaxForm" loader="true" method="post"
                                      action="<?php echo site_url(route_to('teacher.academic.yearly_certificate.save_homeroom_teacher_comment', $student->id)); ?>">
                                        ...
                                        <div class="form-group">
                                            <label for="sess">1 <sup>st</sup> Semester Comment &nbsp;&nbsp;&nbsp;&nbsp;
                                                <span style="text-align: right;margin-left: 32%;color: red;font-weight: 800">
                                                 ...
                                                 if (isset($total_marks[$student->id.'.'.$semesters[0]->id])){
                                                     echo number_format(($total_marks[$student->id.'.'.$semesters[0]->id])/$counter,2);
                                                 }
                                                 ?>
                                            </span>
                                            </label>
                                            ...
                                            <div style="margin-left: 65%;margin-top: 2%">
                                                <input type="checkbox" id="sign1" name="sign1" value="1" required <?php if(!empty($home)){?> checked<?php }?>>
                                        ...
                                        <div class="form-group">
                                            <label for="sess">2 <sup>nd</sup> Semester Comment &nbsp;&nbsp;&nbsp;&nbsp;
                                                <span style="text-align: right;margin-left: 32%;color: red;font-weight: 800">
                                                 ...
                                                 if (isset($total_marks[$student->id.'.'.$semesters[1]->id])){
                                                     echo number_format(($total_marks[$student->id.'.'.$semesters[1]->id])/$counter,2);
                                                 }
                                                 ?>
                                            </span>
                                            </label>
                                            ...
                                            <div style="margin-left: 65%;margin-top: 2%">
                                                <input type="checkbox" id="sign1" name="sign1" value="1" <?php if(!empty($home)){?> checked<?php }?>>
                                                <label style="font-size: 16px"> Sign</label>
                                            </div>
                                        </div>
                                    </div>
                ...
                <td style="width: 10%" class="">
                    <input type="checkbox" name="homeroom_sign" class="director_sign" value="<?php echo $student->id;?>" <?php if (isset($home) && ($home->is_signed==1)){?> checked <?php }?>>
                </td>
            </tr>
            <?php
        }
...
<script>
    $(function (){
        $("#checkAll").click(function(){
            ...
            var url = '<?php echo site_url(route_to('teacher.academic.yearly_certificate.homeroom_sign')); ?>';
            var data = {
