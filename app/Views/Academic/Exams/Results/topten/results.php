...
if($exam && $section) {
   ...
   foreach ($rank_students as $key => $value){
       if ($value ==1 || $value == 2 || $value == 3 || $value == 4 || $value == 5 || $value ==6 || $value == 7 || $value == 8 || $value == 9 || $value == 10){
           array_push($top10_arr, array('id'=>$key,'rank'=>$value));
       }
   }
usort($top10_arr, function ($a, $b) {return $a['rank'] > $b['rank'];});
    ?>
<style>
    .wrapper {
        overflow: hidden;
        ...
        <div class="col-md-9">
         <table style="text-decoration: underline">
           <tr>
             ...
             <tr>
             <th><h1><?php echo getSession()->name.' , '.$exam->name,', '.$students[0]->class->name.' - '.$students[0]->section->name;?></h1></th>
             </tr>
             <tr>
                 <th><h1  class="fs30">Top 10 Students</h1></th>
             </tr>
         </table>
        </div>
    </div>
    <?php
    if($top10_arr && count($top10_arr) > 0) {
        ?>
       <div class="row" style="margin-top: 2%">
           <?php foreach ($top10_arr as $key => $value):
