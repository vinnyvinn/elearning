        ...
        <div class="header-body">
            <div class="row align-items-center py-4">
                <div class="col-lg-6 col-7">
                    <h6 class="h2 text-white d-inline-block mb-0">Exam Results</h6><br/>
                </div>
    ...
    foreach ($students as $student) {
        foreach ($subjects as $subject) {
            $result = (new ExamResults())->select('SUM(mark) as subtotal')->where('student', $student->id)->where('class', $class->id)
                ->where('subject', $subject->id)->where('exam', $exam->id)->orderBy('created_at','DESC')->get()->getRowObject();
            ...
            <table class="table" id="results_table">
                ...
                foreach ($rank_students as $student => $rank) {
                    ...
                    <tr>
                        ...
                        $big = $model->select('SUM(mark) as tt')->where(['exam' => $exam->id, 'student' => $student, 'class' => $class->id])->get()->getRowObject();
                        $tt = $big->tt;
                        $total_marks = 0;
                        foreach ($subjects as $subject) {
                         //  if ($subject->optional !=1) {
                                $rs = $model->where(['exam' => $exam->id, 'student' => $student, 'class' => $class->id, 'subject' => $subject->id])->get()->getRowObject();
                                ?>
