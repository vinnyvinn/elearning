...
$teacher = $student->section->advisor;
?>
                ...
                <div class="card-body">
                    <div class="mb-1">
                        <div style="max-height: 500px; overflow-y: scroll" id="messageArea" class="p-2 pb-3">
                            <?php
                            $messages = (new Messages())->where('session',active_session())->where(['teacher' => $teacher->id])->where('parent',$parent->id)->orderBy('id', 'ASC')->limit(100)->findAll();
                           // var_dump($teacher);
                         //   exit();
                        ...
                        <div class="media align-items-center">
                            ...
                            <div class="media-body" style="display: none">
                                <form method="post" class="ajaxForm"
                                      action="<?php echo site_url(route_to('admin.message.send')); ?>" loader="true">
                                    <input type="hidden" name="teacher" value="<?php echo $teacher->id; ?>"/>
                                    <input type="hidden" name="parent" value="<?php echo $parent->id; ?>"/>
                                    <input type="hidden" name="for_student" value="<?php echo $student->id; ?>"/>
...
<script>
    ...
    function reloadMessages() {
        var d = {
            loader: false,
            data: "parent=<?php echo $parent->id; ?>&teacher=<?php echo $teacher->id; ?>&student=<?php echo $student->id; ?>",
            url: "<?php echo site_url(route_to('admin.message.ajax_fetch')); ?>"
        };
