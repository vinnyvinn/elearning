        ...
        <ul class="nav nav-pills nav-pill-bordered">
            ...
            foreach ($students as $student):?>
                <li class="nav-item">
                    ...
                    <a class="walla nav-link <?php if ($active->id == $student->id):?>active<?php endif;?>" id="base-pill<?php echo $student->id;?>" data-toggle="pill" href="#pill<?php echo $student->id;?>" aria-expanded="true" student-id="<?php echo $student->id;?>" value="0999"><?php echo $student->profile->name;
                        echo '<br>';
...
<script>
 ...
 $(function (){
   $('.std').val($('.walla.active').attr('student-id'));

     $('.walla').on('click',function (){
         $('.std').val($(this).attr('student-id'));

         var d = {
             url: "<?php echo site_url(route_to('parent.attendance.students-details.ajax')); ?>",
             data: 'student='+$(this).attr('student-id'),
             loader: true
         }
         ajaxRequest(d, function (resp) {
             let data = JSON.parse(resp);
             if (data !==undefined){
               attendanceDetails(data.year,data.month,data.student);
             }else {
                 console.log('hide')
             }
 ...
 var student = $('.walla.active').attr('student-id');
