...
?>
                    ...
                    <div class="table-responsive">
                        <table class="table">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($subject->optional == 0):
                                            ?>
                                            <?php echo isset($data_arr[$subject->id]) ? getScore($grading,$data_arr[$subject->id] / count($semesters)) : '-';?>
                                        <?php else:?>
                                            <?php
                            ...
                            <tr>
                                ...
                                foreach ($semesters as $item) {
                                    ?>
                                    <th><?php echo getScore($grading,number_format($total_marks[$item->id]/$count,2)); ?></th>
                                    <?php
                                }
                                ?>
                                <td><?php echo  getScore($grading,number_format($tt/$count,2));?></td>
                                <td></td>
                            </tr>
                            <tr>
                                <td></td>
            ...
            <div style="text-align: right">
                <a href="javascript:void(0)" class="previous_kg_c">&laquo; Previous </a>
                <a href="javascript:void(0)" class="next_kg_c">Next &raquo;</a>
            </div>
        </div>
<?php endif;?>
...
<script>
    ...
    $('.next_kg_c').on('click',function (){
        next(sess,student);
    })
    $('.previous_kg_c').on('click',function (){
        back(sess,student);
    })
    function next(session,student) {
        var d = {
            url: "<?php echo site_url(route_to('student.certificate.evaluation')); ?>",
            loader: true,
            data: "year="+session+"&student="+student
        };
        ajaxRequest(d, function (data) {
            $('.certificate_kg').html(data);
        })
    }
